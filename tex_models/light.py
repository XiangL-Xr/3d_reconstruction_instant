...
## Split-Sum environment map light source automatic mipmap generation ===========================
class EnvironmentLight(torch.nn.Module):
    ...
    def shade(self, gb_pos, gb_normal, kd, ks, view_pos, specular=True):
        wo = utils.safe_normalize(view_pos - gb_pos)
        if specular:
        ...
        reflvec = utils.safe_normalize(utils.reflect(wo, gb_normal))
        nrmvec  = gb_normal
        ...
        if specular:
            ...
            NdotV = torch.clamp(utils.dot(wo, gb_normal), min=1e-4)
            fg_uv = torch.cat((NdotV, roughness), dim=-1)
            if not hasattr(self, '_FG_LUT'):
                self._FG_LUT = torch.as_tensor(np.fromfile('./tex_models/render/bsdf_256_256.bin', dtype=np.float32).reshape(1, 256, 256, 2), dtype=torch.float32, device='cuda')
            fg_lookup = dr.texture(self._FG_LUT, fg_uv, filter_mode='linear', boundary_mode='clamp')
            
            # roughness adjusted specular env lookup
            miplevel = self.get_mip(roughness)
            spec = dr.texture(self.specular[0][None, ...], reflvec.contiguous(), mip=list(m[None, ...] for m in self.specular[1:]), mip_level_bias=miplevel[..., 0],
...
class cubemap_mip(torch.autograd.Function):
    ...
    def forward(ctx, cubemap):
        return utils.avg_pool_nhwc(cubemap, (2, 2))
    ...
    def backward(ctx, dout):
        ...
        for s in range(6):
            ...
            v = utils.safe_normalize(utils.cube_to_dir(s, gx, gy))
...
def _load_env_hdr(fn, scale=1.0):
    latlong_img = torch.tensor(utils.load_image(fn), dtype=torch.float32, device='cuda') * scale
    cubemap = utils.latlong_to_cubemap(latlong_img, [512, 512])
    lgt = EnvironmentLight(cubemap)
    lgt.build_mips()
...
def save_env_map(fn, light):
    ...
    if isinstance(light, EnvironmentLight):
        color = utils.cubemap_to_latlong(light.base, [512, 512])
    utils.save_image_raw(fn, color.detach().cpu().numpy())