...
#             img_path = os.path.join(self.config.root_dir, f"{frame['file_path']}.JPG")     ## endswitch:[JPG, jpg, png]
...
class BlenderDatasetBase():
    def setup(self, config, split):
        ...
        for i, frame in enumerate(meta['frames']):
            ...
            img_path = os.path.join(self.config.root_dir, f"{frame['file_path']}.JPG")     ## endswitch:[JPG, jpg, png]
            msk_path = os.path.join(self.config.root_dir, f"{frame['file_path']}.JPG").replace("images", "masks")
            ...
            if self.use_mask:
                mask = Image.open(msk_path).convert('L') # (H, W, 1)
